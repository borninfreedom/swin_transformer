...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q = q * self.scale
        tmp_val0=k.transpose(-2, -1)
        print(f'{tmp_val0.shape=}')
        attn = (q @ tmp_val0)

        tmp_val1=self.relative_position_index.view(-1)
        print(f'{tmp_val1.shape=}')
        print(f'{tmp_val1=}')
        print(f'{self.relative_position_bias_table.shape=}')
        print(f'{self.relative_position_bias_table=}')
        print(f'{self.relative_position_bias_table[tmp_val1].shape=}')
        relative_position_bias = self.relative_position_bias_table[tmp_val1].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        print(f'{relative_position_bias.shape=}')
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        print(f'after permute,{relative_position_bias.shape=}')
        ...
        print(f'{attn @ v.shape=}')
        print(f'{(attn @ v).transpose(1, 2).shape=}')
